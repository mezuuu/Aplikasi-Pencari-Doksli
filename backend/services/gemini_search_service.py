...
class _GeminiKeyManager:
    ...
    def _load_keys(self):
        ...
        if keys_str:
            candidates = [k.strip() for k in keys_str.split(',') if k.strip() and k.strip() not in ('your-gemini-api-key-here', '')]
        ...
        if self._keys:
            logger.info(f'[Gemini KeyManager] Loaded {len(self._keys)} API key(s)')
        else:
            ...
            for attempt in range(total):
                ...
                if exhausted_at is not None:
                    ...
                    if elapsed < QUOTA_COOLDOWN_SECONDS:
                        ...
                        logger.debug(f'[Gemini KeyManager] Key [{idx + 1}/{total}] still cooling down ({remaining}s left)')
                        continue
                    else:
                        logger.info(f'[Gemini KeyManager] Key [{idx + 1}/{total}] cooldown expired, retrying')
                        del self._exhausted[key]
                client = genai.Client(api_key=key)
                self._current_idx = idx
                logger.debug(f'[Gemini KeyManager] Using key [{idx + 1}/{total}]')
                return (client, key)
            logger.warning(f'[Gemini KeyManager] All {total} key(s) are rate-limited. Will retry after cooldown expires.')
            return (None, None)
    ...
    def mark_exhausted(self, api_key):
        ...
        with self._lock:
            ...
            logger.warning(f'[Gemini KeyManager] Key marked as rate-limited ({exhausted_count}/{total} keys exhausted). Auto-retry in {QUOTA_COOLDOWN_SECONDS // 60} min.')
    ...
    def mark_auth_failed(self, api_key):
        ...
        with self._lock:
            if api_key in self._keys:
                ...
                logger.warning(f'[Gemini KeyManager] Permanently removed invalid key. {len(self._keys)} key(s) remaining.')
            if not self._keys:
                self._all_failed = True
                logger.warning('[Gemini KeyManager] All keys permanently disabled.')
    ...
    def status(self):
        ...
        return {'total_keys': total, 'available': available, 'exhausted': exhausted, 'all_failed': self._all_failed}
...
class GeminiSearchService:
    ...
    """
        result = {'keywords': '', 'description': '', 'document_type': '', 'potential_sources': []}
        if not GeminiSearchService.is_gemini_available():
            return result
        try:
        ...
        try:
            ...
            return result
        mime_type = GeminiSearchService._detect_mime_type(image_path)
        ...
        for attempt in range(max(1, total_keys)):
            ...
            if not client:
                ...
                break
            try:
                response = client.models.generate_content(model='gemini-2.0-flash', contents=[image_part, prompt])
                if not response or not response.text:
                    logger.warning('[Gemini] Empty response')
                    return result
                text = response.text.strip()
                desc_match = re.search('DESCRIPTION:\\s*(.+?)(?=\\nDOCUMENT_TYPE:|\\Z)', text, re.DOTALL)
                type_match = re.search('DOCUMENT_TYPE:\\s*(.+?)(?=\\nKEYWORDS:|\\Z)', text, re.DOTALL)
                keywords_match = re.search('KEYWORDS:\\s*(.+?)(?=\\nSOURCES:|\\Z)', text, re.DOTALL)
                sources_match = re.search('SOURCES:\\s*(.+?)$', text, re.DOTALL)
                if desc_match:
                ...
                if keywords_match:
                    ...
                    kws = [k.strip().strip('\'"') for k in raw.split(',') if k.strip() and k.strip() not in ('...', 'N/A', 'none', '-')]
                    result['keywords'] = ' '.join(kws[:8])
                if sources_match:
                    raw = sources_match.group(1).strip().strip('[]')
                    srcs = [s.strip().strip('\'"') for s in raw.split(',') if s.strip() and s.strip() not in ('...', 'N/A', 'none', '-', 'None', 'Unknown')]
                    result['potential_sources'] = srcs[:3]
                logger.info(f"[Gemini] Analysis OK — type='{result['document_type']}', kw='{result['keywords'][:50]}'")
                return result
    ...
    """
        result = {'urls': [], 'search_queries': [], 'description': '', 'grounding_sources': []}
        if not GeminiSearchService.is_gemini_available():
            return result
        try:
            from google.genai import types
        ...
        for attempt in range(max(1, total_keys)):
            ...
            if not client:
                ...
                break
            try:
                response = client.models.generate_content(model='gemini-2.0-flash', contents=[image_part, prompt], config=types.GenerateContentConfig(tools=[google_search_tool]))
                if not response or not response.text:
                                    ...
                                    if chunks and idx < len(chunks):
                                        ...
                                        if web:
                                            ...
                                            if uri:
                                                found_urls.append(uri)
                seen = set()
                skip_patterns = ['google.com/search', 'googleapis.com', 'gstatic.com', 'googleusercontent.com/s2', 'schema.org']
                clean_urls = []
                ...
                logger.info(f"[Gemini+Search] Found {len(clean_urls)} URLs, {len(result['grounding_sources'])} grounded, queries={result['search_queries']}")
                return result
            except Exception as e:
                error_msg = str(e).lower()
                if any((kw in error_msg for kw in ['401', '403', 'unauthorized', 'invalid', 'api_key'])):
    ...
    """
        search_result = GeminiSearchService.gemini_google_search_image(image_path, max_results)
        analysis = {'keywords': '', 'description': search_result.get('description', ''), 'document_type': '', 'potential_sources': [], 'google_search_urls': search_result.get('urls', []), 'grounding_sources': search_result.get('grounding_sources', []), 'search_queries': search_result.get('search_queries', [])}
        ...
        if urls:
            ...
            logger.info(f'[Gemini] Google Search found {len(urls)} URLs directly')
            return (search_query, urls, analysis)
        logger.info('[Gemini] Google Search returned no URLs, falling back to keyword extraction')
        desc_result = GeminiSearchService.gemini_describe_image(image_path)
        analysis['keywords'] = desc_result.get('keywords', '')
        analysis['description'] = analysis['description'] or desc_result.get('description', '')
        ...
        if not analysis['keywords']:
            ...
            return ('', [], analysis)
        query_parts = []
        if analysis['document_type'] and analysis['document_type'].lower() not in ('photo', 'graphic', 'image'):
