...
def generate_crop_regions(image_path):
    ...
    # Prepare temporary directory for crops
    ...
    for scale in scales:
        ...
        # Skip if window is too small to contain meaningful content
        if win_w < min_size or win_h < min_size:
            logger.debug(
                f"Skipping scale {scale}: window {win_w}x{win_h} "
                f"below minimum {min_size}px"
            )
            continue
    ...
    logger.info(
        f"Generated {len(crop_paths)} crop regions from {image_path} "
        f"(original: {img_w}x{img_h}, scales: {scales}, overlap: {overlap})"
    )
